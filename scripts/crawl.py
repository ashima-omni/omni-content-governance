...
MODEL_FILTER = os.environ.get("MODEL_FILTER", "").strip()    # model UUID: crawl only its documents
...
def crawl() -> list[dict]:
    ...
    if MODEL_FILTER:
        body = get(f"/models/{MODEL_FILTER}/content-validator",
                   {"include_personal_folders": "true"})
        model_doc_ids = {d.get("identifier") or d.get("document_id")
                         for d in body.get("content", [])}
        docs = [d for d in docs if d["identifier"] in model_doc_ids]
        print(f"Model filter {MODEL_FILTER}: {len(docs)} documents on this model")
    ...
    for i, doc in enumerate(docs, 1):
        ...
        for q in body.get("queries", []):
            ...
            if MODEL_FILTER and query.get("modelId") not in (MODEL_FILTER, None):
                continue
