...
def load_snowflake(database: str, schema: str) -> None:
    ...
    ddl = {
        "LINEAGE": """create table if not exists {q}.LINEAGE (
            SNAPSHOT_DATE date, DOC_ID varchar, DOCUMENT varchar,
            FOLDER_PATH varchar, OWNER_NAME varchar, DOC_VIEWS number,
            DOC_FAVORITES number, UPDATED_AT timestamp_ntz, TILE varchar,
            TILE_URL varchar, TILE_HASH varchar, TOPIC varchar,
            VIEW_NAME varchar, FIELD_NAME varchar, MODEL_ID varchar)""",
        "DASHBOARD_SIMILARITY": """create table if not exists {q}.DASHBOARD_SIMILARITY (
            SNAPSHOT_DATE date, DOC_ID_A varchar, DOC_NAME_A varchar,
            DOC_ID_B varchar, DOC_NAME_B varchar, SHARED_TILES number,
            TILES_A number, TILES_B number, SIMILARITY float,
            FIELD_SIMILARITY float, VERDICT varchar)""",
    }
    qualified = f'"{database}"."{schema}"'
    if os.environ.get("RESET_TABLES") == "1":
        for t in ddl:
            cur.execute(f"drop table if exists {qualified}.{t}")
        print("RESET_TABLES=1: dropped existing governance tables")
    for t, stmt in ddl.items():
        cur.execute(stmt.format(q=qualified))
